...
const getTenderLists = async (payload)=>{
    ...
    const query = `select(
        ...
        case 
        when TBL.IsPublished is null and TBL.IsSale is null then 'Not Published'
        when TBL.IsPublished=1 and TBL.IsSale=0 then 'On Going'
        else 'Sold'
        end as Status,
        (
            Select 
            ...
            where AA.TenderId=A.TenderId and BB.IsActive=1 for json path
        ) as details
        from Tender A
        left join TenderBidLists TBL on A.TenderId=TBL.TenderId
        where A.IsActive=1 order by A.TenderId desc OFFSET ${Skip} ROWS 
...
const getCount = async()=>{
    const query = `select Count(TenderId) as count from Tender where IsActive=1`;
    const data = await getData(dbConfig3, query);
    return data[0].count;
}
