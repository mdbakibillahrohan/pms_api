...
const getTenderLists = async (payload)=>{
    ...
    ROW_NUMBER() OVER (ORDER BY (SELECT 1)) AS [key],
       A.TenderBidId,A.Description,B.TenderNo,B.TenderNo+' - '+B.TenderTitle as TenderNoTitle,B.MinimumBidAmount,A.OpenDate,
    ...
    where 
    A.IsDeleted=0 
    group by A.TenderBidId,A.Description,B.TenderNo,B.TenderTitle,B.MinimumBidAmount,A.OpenDate,A.CloseDate,A.IsSale
