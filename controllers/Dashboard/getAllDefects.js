...
const GetAllDefectsController = async (req, res) => {
  ...
  try {
    ...
    const sql=`select top (10) f.DefectName as name,sum(CountValue) as value from HourlySewingDefectCount D with(nolock)
    ...
    //  const sql=`select top (10) f.DefectName as name,sum(CountValue) as value from HourlySewingDefectCount D
    // inner join IE_Defects f on f.DefectId = D.DefectID
    // where CAST(CreateAt as date) = CAST('2023-9-7' as date) and D.UnitId = ${UnitId}
    // group by f.DefectName order by sum(CountValue) desc`;
...
module.exports = GetAllDefectsController;
