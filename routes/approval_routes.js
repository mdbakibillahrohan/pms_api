...
const { controller: rejectController, schema: rejctSchema } = require('../controllers/challan_approve/reject_challan_controller');
...
approvalRouter.post(API.API_CONTEXT + API.APPROVE_CHALLAN, validator(approveSchema), authenticationMiddleware, approvalAuthorizationMiddleware, approveController);
approvalRouter.post(API.API_CONTEXT + API.REJECT_CHALLAN, validator(rejctSchema), authenticationMiddleware, approvalAuthorizationMiddleware, rejectController);
approvalRouter.post(API.API_CONTEXT + API.GET_CHALLAN_LIST, validator(challanListSchema), authenticationMiddleware, approvalAuthorizationMiddleware, challanListController);
approvalRouter.post(API.API_CONTEXT + API.GET_CHALLAN_SUMMARY, validator(challanSummarySchema), authenticationMiddleware, approvalAuthorizationMiddleware, challanSummaryController);
